...
def generate_space_layout(program: ArchitecturalProgram, graph: SpatialGraph, alternative_id: str, family: str = "COURTYARD") -> dict[str, Any]:
    ...
    for index, req in enumerate(program.requirements):
        ...
        for repeat in range(req.quantity):
            ...
            elements.append(SpatialElement(sid, "SPATIAL_ZONE", _rect(x + repeat * 2, y + repeat * 2, width, height), level=0, labels={"name": req.name, "space_type": req.space_type}, metadata={"program_requirement_id": req.id, "space_id": sid, "alternative_id": alternative_id, "area_range": list(req.area_range), "generated_area_m2": width * height, "height": 3.2, "base_elevation": 0.0, "conceptual_volume_m3": width * height * 3.2, "vertical_extent_status": "CONCEPTUAL_DERIVED", "generation_operation": "GENERATE_SPACE_LAYOUT", "family": family, "provenance": req.provenance, "status": "GENERATED"}))
    ...
    return {"representation": representation.to_dict(), "space_ids": [item.id for item in elements if item.element_type == "SPATIAL_ZONE"], "circulation_ids": [access_id], "lineage": {"parent": "BASE_PROGRAM", "operation": "GENERATE_SPACE_LAYOUT", "alternative_id": alternative_id}, "relationship_evaluation": [{"relationship_id": edge.id, "state": "UNKNOWN", "reason": "Generated layout requires geometric validation"} for edge in graph.edges], "validation": "GENERATED_LAYOUT_NOT_VALIDATED"}
